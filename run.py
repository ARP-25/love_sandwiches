...
def validate_data(values):
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                f"Exactly 6 values required, you priveded {len(values)}"
            )
...
def update_worksheet(data, worksheet):
    """
    Update chosen worksheet, add new row with the data list provided
    """
...
def get_last_five_entries_sales():
    ...
    the last 5 entries for each sandwich anmd returns the data
    as a list of lists
    """
